...
import csv #Searchable stock list csv
...
def load_top_500_stocks():
    global top_500_stocks
    top_500_stocks = []

    try:
        folder_path = os.path.dirname(os.path.abspath(__file__))
        csv_path = os.path.join(folder_path, "top_500_stocks.csv")
        ...
        with open(csv_path, "r", newline="", encoding="utf-8-sig") as file:
            ...
            for row in reader:
                ...
                if ticker and company:
                    top_500_stocks.append({
                        "ticker": ticker.strip().upper(),
                        "company": company.strip()
                    })

    except Exception as e:
        pass #Stock list could not be loaded, search will be unavailable
...
def show_stock_input_screen():
    ...
    # Search company section (Written by Group)
    if top_500_stocks:
        search_text = st.text_input("Search Company (type to filter):")
        if search_text:
            matches = [
                f"{s['ticker']} - {s['company']}" for s in top_500_stocks
                if search_text.lower() in s["ticker"].lower() or search_text.lower() in s["company"].lower()
            ][:10]
            if matches:
                selected = st.selectbox("Select a result to autofill ticker:", [""] + matches)
                if selected:
                    st.session_state._autofill_ticker = selected.split(" - ")[0]
            else:
                st.caption("No matches found.")

    #Input row for ticker and amount
    autofill = st.session_state.pop("_autofill_ticker", "") if "_autofill_ticker" in st.session_state else ""
    col1, col2, col3 = st.columns([2, 2, 1])
    with col1:
        ticker_input = st.text_input("Stock Ticker (e.g. AAPL)", value=autofill).strip().upper()
    with col2:
